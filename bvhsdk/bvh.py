        ...
        for section in ['header', 'content']:
            if section == 'header':
                ...
                for frame in range(animation.frames):
                    line = []
                    for joint in animation.getlistofjoints():
                        if joint.rotation.shape[0] <= frame or joint.translation.shape[0] <= frame:
                            print("WARNING: Number of frames in joint {0} ({1} frames) is different from declared in file ({2} frames).".format(joint.name, joint.rotation.shape[0], animation.frames))
                        if writeTranslation or joint==animation.root:
